...
input_dataset = dsets.ImageFolder(root='Images_enhanced/', transform=transform)

data_loader = torch.utils.data.DataLoader(dataset=input_dataset,
...
def plot_result(generator, noise, num_epoch, save=True, save_dir='DCGAN_results/', show=True, fig_size=(5, 5)):
    generator.eval()
...
# Check if CUDA is available and if not, run on CPU
...
for epoch in range(num_epochs):
    ...
    for i, (images, _) in enumerate(data_loader):
        ...
        # labels
        y_real_ = Variable(torch.ones(mini_batch,11,11).to(device))
        ...
        D_fake_decision = D(gen_image).squeeze()
        G_loss = criterion(D_fake_decision, Variable(torch.ones(mini_batch).to(device)))
...
for epoch in range(num_epochs):
    ...
    save_fn1 = save_dir + 'DCGAN_losses_epoch_{:d}'.format(epoch + 1) + '.png'
    loss_plots.append(imageio.imread(save_fn1))

    save_fn2 = save_dir + 'DCGAN_epoch_{:d}'.format(epoch + 1) + '.png'
    gen_image_plots.append(imageio.imread(save_fn2))

imageio.mimsave(save_dir + 'DCGAN_losses_epochs_{:d}'.format(num_epochs) + '.gif', loss_plots, fps=5)
imageio.mimsave(save_dir + 'DCGAN_epochs_{:d}'.format(num_epochs) + '.gif', gen_image_plots, fps=5)